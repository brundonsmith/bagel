            ...
            switch(current.kind) {
                ...
                case "invocation": {
                    ...
                    if (current.kind === "invocation" && current.subject.kind === "local-identifier") {
                        const binding = getBinding(() => {}, current.subject.name, current.subject)
                ...
                case "local-identifier": {
                    ...
                    getBinding(reportError, current.name, current)
                } break;
                case "inline-const":
                ...
                case "assignment": {
                    const resolved = current.target.kind === "local-identifier" ? getBinding(reportError, current.target.name, current.target) : undefined
...
export function subsumes(passthrough: Passthrough, destination: TypeExpression, value: TypeExpression): boolean {
    ...
    if (
        ...
        return subsumes(passthrough, resolvedDestination.inner, resolvedValue.inner);
...
export function resolveType(passthrough: Passthrough, type: TypeExpression): TypeExpression {
    ...
    switch (type.kind) {
        case "named-type": {
            const binding = getBinding(reportError, type.name.name, type.name)
